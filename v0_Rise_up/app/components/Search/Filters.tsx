...
const mockFacets: Facets = {
  sources: ["twitter", "telegram", "perplexity"],
  topics: [
    "iran.now",
    "iran.statements_official",
    "iran.statements_opposition",
    "leaders.world_statements",
    "protests.solidarity_global",
    "top.breaking_global",
  ],
  channels: ["FreedomNews", "IranWatch", "ResistanceTelegram"],
}

export default function Filters({ filters, onChange, onReset }: FiltersProps) {
  const [facets] = useState<Facets>(mockFacets)
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {activeFilterCount > 0 && (
            <span className="ml-2 px-2 py-0.5 bg-blue-600 text-white text-xs rounded-full">{activeFilterCount}</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          {activeFilterCount > 0 && (
